...
namespace Manager
{
	public class ResourceManager
	{
		...
		{
			_pickPoses.Clear();
			for (var i = 1; i <= Tools.Constants.MaxPlayer; i++)
			{
		...
		public void LoadMahjong()
		{
			...
			// }
			for (var j = 0; j < Tools.Constants.MaxPlayer - 1; j++)
			{
				_mahjongList.Add(
					new Mahjong(25, "mahjong_tile_" + 25));
			}
			...
			for (var i = 29; i <= Tools.Constants.MaxId; i++)
			{
				for (var j = 0; j < Tools.Constants.MaxPlayer; j++)
				{
					_mahjongList.Add(
		...
		public GameObject GeneratePlayer(int id)
		{
			...
			go.name = $"{GameManager.Instance.GetPlayerName()}";
			go.GetComponent<PlayerController>().SetName(id + 1, GameManager.Instance.GetPlayerName());
